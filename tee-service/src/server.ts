...
function deriveKeyMaterial(secret: string) {
  return webcrypto.subtle.digest('SHA-256', textEncoder.encode(secret));
}

async function encryptPayload(plainText: string) {
...
async function decryptPayload(ivB64: string, cipherB64: string) {
...
function makeStoragePath(cid: string) {
  return path.join(STORAGE_DIR, `${cid}.json`);
}

app.post('/encrypt', async (req, res) => {
  try {
    const { recipientEmail, poolAddress, sellerAddress, assetId } = req.body as {
    ...
    await fs.writeFile(
      makeStoragePath(encryptedCID),
      JSON.stringify({ encryptedCID, assetId: canonicalId, dataHash, ...encrypted }, null, 2),
      'utf8'
    );
...
app.post('/decrypt', async (req, res) => {
  try {
    const { encryptedCID } = req.body as {
      encryptedCID?: string;
      buyerAddress?: string;
      signature?: string;
    };
    ...
    const payload = JSON.parse(raw) as { iv: string; cipher: string };
...
app.post('/decrypt-batch', async (req, res) => {
  try {
    const { encryptedCIDs } = req.body as { encryptedCIDs?: string[] };
